...
export const SYSTEM_INSTRUCTION = `Você é um assistente virtual especializado em gerenciar "Cartões de Território" para um trabalho de visitação.
...
export const tools: FunctionDeclaration[] = [
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        name: { type: Type.STRING, description: 'Nome do bairro' }
      },
      required: ['name']
    }
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        id: { type: Type.STRING, description: 'ID do bairro' }
      },
      required: ['id']
    }
  },
  ...
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        id: { type: Type.STRING, description: 'ID do território' }
      },
      required: ['id']
    }
  },
  ...
  },
  {
    ...
    parameters: {
      type: Type.OBJECT,
      properties: {
        id: { type: Type.STRING, description: 'ID do endereço' }
      },
      required: ['id']
    }
  },
...
export async function sendMessageToAI(
  ...
  let locationContext = '';
  if (db.city || db.state) {
    locationContext = `\n\nLOCALIZAÇÃO BASE DO USUÁRIO: ${db.city || ''} - ${db.state || ''}. Use isso como contexto geográfico se necessário.`;
  }
  
  // Create a copy of the database without the chats history to save tokens
  const dbForAI = {
    bairros: db.bairros,
    city: db.city,
    state: db.state
